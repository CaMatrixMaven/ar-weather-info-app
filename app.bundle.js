  ...
  function safeInit() {
    try {
      ...
      if (
        ...
        console.error("DOM init failed: missing required elements");
        alert("App failed to initialize: missing UI elements.");
        return;
      }
      ...
      const CACHE_KEY = 'weather-cache-v3';
      ...
      async function handleStartAR() {
        ...
        try {
          ...
          debug(`Version: ${window.__APP_VERSION__ || 'unknown'}`);
      ...
      console.error("SAFE INIT FAILED", err);
      alert("Fatal startup error: " + (err?.message || err));
    }
  }
